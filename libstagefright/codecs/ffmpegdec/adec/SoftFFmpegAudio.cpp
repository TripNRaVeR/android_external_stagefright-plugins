...
SoftFFmpegAudio::SoftFFmpegAudio(
        ...
        OMX_PTR appData,
        OMX_COMPONENTTYPE **component) : SimpleSoftOMXComponent(name, callbacks, appData, component),
      mMode(MODE_NONE),
...
OMX_ERRORTYPE SoftFFmpegAudio::internalGetParameter(OMX_INDEXTYPE index, OMX_PTR params) {
...
void SoftFFmpegAudio::adjustAudioParams() {
    ...
    // channels support 1 or 2 only (hardcoded in audiosystem)
    channels = mCtx->channels >= 2 ? 2 : 1;

    // prevent going below or above supported rates
...
OMX_ERRORTYPE SoftFFmpegAudio::internalSetParameter(OMX_INDEXTYPE index, const OMX_PTR params) {
    ...
    if (mIgnoreExtradata) {
        ...
        if (!mExtradataReady) {
            ...
            if (mCtx->codec_id == AV_CODEC_ID_VORBIS) {
                ...
                memcpy(mCtx->extradata + orig_extradata_size, inHeader->pBuffer + inHeader->nOffset, inHeader->nFilledLen);
...
void SoftFFmpegAudio::initPacket(AVPacket *pkt, OMX_BUFFERHEADERTYPE *inHeader) {
...
int32_t SoftFFmpegAudio::decodeAudio() {
    ...
    if (len < 0) {
        ALOGW("ffmpeg audio decoder err, skip the frame and play silence instead");
...
int32_t SoftFFmpegAudio::resampleAudio() {
    ...
    dataSize = av_samples_get_buffer_size(NULL, av_frame_get_channels(mFrame), mFrame->nb_samples,
                                                                               (enum AVSampleFormat)mFrame->format, 1);

    channels = av_get_channel_layout_nb_channels(mFrame->channel_layout);
    channelLayout = (mFrame->channel_layout && av_frame_get_channels(mFrame) == channels) ?
                     mFrame->channel_layout : av_get_default_channel_layout(av_frame_get_channels(mFrame));

    if (mFrame->format != mAudioSrcFmt || channelLayout != mAudioSrcChannelLayout
        ...
        mSwrCtx = swr_alloc_set_opts(NULL, mAudioTgtChannelLayout, mAudioTgtFmt, mAudioTgtFreq, channelLayout,
                                                                   (enum AVSampleFormat)mFrame->format, mFrame->sample_rate,0, NULL);
        ...
        av_get_channel_layout_string(src_layout_name, sizeof(src_layout_name), mCtx->channels, channelLayout);
        av_get_channel_layout_string(tgt_layout_name, sizeof(tgt_layout_name), mAudioTgtChannels, mAudioTgtChannelLayout);
